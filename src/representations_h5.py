...
def repr_cosine_centered(contours, **kwargs):
    """Centered contours (with mean pitch 0) represented as cosine contours"""
    return dct(repr_pitch_centered(contours))

def repr_cosine_normalized(contours, **kwargs):
    """Normalized contours (minimum pitch 0, maximum pitch 1) represented
    as cosine contours"""
    return dct(repr_pitch_normalized(contours))

def repr_cosine_tonicized(contours, **kwargs):
    """Represent tonicized contours (tonic has pitch 0) as cosine contours"""
    return dct(repr_pitch_tonicized(contours, **kwargs))

def repr_cosine_finalized(contours, **kwargs):
    """Represent finalized contours (final has pitch 0) as cosine contours"""
    return dct(repr_pitch_finalized(contours, **kwargs))
