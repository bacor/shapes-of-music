...
def poisson_segmentation(
    iterable: list, lam: float, omit_first_and_last: bool = True
) -> list:
    ...
    """
    if lam < 1:
        raise ValueError("Lambda should be > 1.")
    segments = []
...
def extract_random_segments_from_file(
    filepath: str, lam: float, omit_first_and_last: bool = True
) -> list:
    ...
    segments = poisson_segmentation(
        elements, lam=lam, omit_first_and_last=omit_first_and_last
    )
