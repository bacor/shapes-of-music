...
def extract_volpiano_contours(chants, df, segmentation, max_num_contours=20000,
    num_samples=50, contour_id_tmpl: str = '{i:0>4}', random_state=12345):
    ...
    indices = np.sort(np.random.choice(np.arange(len(units)), size=max_num_contours, replace=False))
    contours = np.array([volpiano_to_contour(unit, num_samples=num_samples) for unit in units[indices]])
    ...
    contours_df = pd.DataFrame(
        ...
        columns = [
            'contour_id', 'chant_id', 
            'unit_num', 'unit_length', 'unit_duration', 
            'tonic_krumhansl', 'tonic_mode', 'final', 'mode'
